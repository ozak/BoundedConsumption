...
# Output directory
dir=os.getenv("HOME")+'/Dropbox/LatexMe/Consumption/data/HOAC/'
...
for i,j in product(range(len(a)),range(len(b))):
    ...
    '''
    P=[dynsysf.phinv(Finv(x,g(wgrid))) for x in wgrid]
    P=np.array(P)
    P=P.transpose()
    Pstat[:,i,j]=stationary(P).transpose()
    '''
    Wealth_srs=SRS(F=F,phi=phi,X=np.random.permutation(wgrid)[0])
    Pstat[:,i,j]=Wealth_srs.stationaryDist(n=t, FInv=Finv, dF=dF, phinv=dynsysf.phinv, dx=dx,xmin=0.1)
    V[:,i,j]=v0(wgrid)
