...
# Let's replicate the Howitt-Özak/Allen-Carroll parameter's
...
# What does the stationary distribution look ike before 100000
...
print "Expected wealth=%1.2f, expected LT Utility=%1.2f, CE=%1.2f" %(EW,EV,CE) 
...
while count<maxiter:
    ...
    if np.mod(count,25)==0:
        ...
        plt.ylim([0,5])
        #plt.xlim([0,2])
        plt.plot(grid,sigma1(grid)+mean(y))
        plt.draw()
        print '%d %2.10f ' % (count,err)
...
plt.draw()
# Expected Wealth and Expected Value if initial wealth is distributed according to the stationary PDF
cons=optcons2(Ws)    # Consumption on Ws
EW2=sum(statprob*Ws) # Expected wealth under stationary distribution
EV2=sum(statprob*v0(Ws)) # Expected Lifetime utility under optimal rule when initial wealth is distributed according to stationary probability
