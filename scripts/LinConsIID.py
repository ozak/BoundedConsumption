...
======================================================
# This code generates the Value Functions generated by linear consumption rules for more general income process in the analysis of the papers:
...
for i,j in product(range(len(a)),range(len(b))):
    ...
    Wealth_srs=SRS(F=F,phi=np.random.lognormal,X=np.random.permutation(grid)[0],mu=0,sigma=sigman)  # Wealth stochastic dynamical system
    Pstat[:,i,j]=Wealth_srs.stationaryDist(n=t, FInv=Finv, dF=dF, phinv=phinv, dx=dx,xmin=0.1)      # Stationary distribution of wealth for each rule
