...
plt.figure(figsize=(10, 6))
plt.xlim(ULin.sum(axis=0).mean()-.5*ULin.sum(axis=0).std(),Uopt.sum(axis=0).max())
plt.hist(ULin.sum(axis=0),bins=50000,histtype='step',color='b',normed=True,weights=1/N*np.ones(N),label=r'$\sum_{t=0}^T \beta^tU(c^b_t(w_t))$')
plt.hist(Uopt.sum(axis=0),bins=500,histtype='step',color='g',normed=True,weights=1/N*np.ones(N),label=r'$\sum_{t=0}^T \beta^tU(c^*(w_t))$')
...
plt.figure(figsize=(10, 6))
plt.xlim((Udif.mean()-.5*Udif.std(),Udif.max()+.1))    
plt.hist(Udif,bins=50000,histtype='step',color='b',normed=True,weights=1/N*np.ones(N),label=r'Utility Difference')
