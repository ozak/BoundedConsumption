...
# coding: utf-8
# This code generates the dynamics of the learning algorithm for the paper under more general income process:
...
y = exp(sigman*randn(N,T))                   # Draws of shock 
...
V=np.zeros_like(CElin)
for i,j in product(range(CElin.shape[0]),range(CElin.shape[1])):
    V[i,j]=(Vopt(wgrid).dot(Pstat[:,i,j])*(1-beta)*theta1+1)**(1/theta1)
    V[i,j]=(1-CElin[i,j]/V[i,j])*100
...
a0=0*np.ones(N)
b0=0.5*np.ones(N)
w0=np.array(Ws[[sample(statprob) for i in range(N)] ])#Ws.mean()*np.ones(N)
...
plt.scatter(ULin.sum(axis=0),Uopt.sum(axis=0))
if alg=='backward':
    plt.xlim(-5,ULin.sum(axis=0).max())
...
if alg=='backward':
    binsx=30000
    binsy=1000
    plt.xlim((-5,max(ULin.sum(axis=0).max(),Uopt.sum(axis=0).max())))
elif alg=='zero':
    binsx=80000
    binsy=300
    plt.xlim((-5,max(ULin.sum(axis=0).max(),Uopt.sum(axis=0).max())))        
...
plt.figure(figsize=(10, 6))   
if alg=='backward':
    plt.xlim((.5,CEOpt.max()))      
elif alg=='zero':
    plt.xlim((0.5,CEOpt.max()))
plt.hist(CELin,bins=1000,histtype='step',color='b',normed=True,label=r'CE Bounded')
plt.hist(CEOpt,bins=1000,histtype='step',color='g',normed=True,label=r'CE Optimal')
...
plt.figure(figsize=(10, 6))    
if alg=='backward':
    plt.xlim(((CELin-CEOpt).mean()-3*(CELin-CEOpt).std(),(CELin-CEOpt).max()))
    posx=2      
elif alg=='zero':
    plt.xlim((-.5,(CELin-CEOpt).max()))
    posx=2  
plt.hist(CELin-CEOpt,bins=1000,histtype='step',color='b',normed=True,label=r'CE Difference')
